...
def parse_args():
    ...
    parser.add_argument('--file-url',
                        type=str,
                        help='output path for the result vector of the gradcam')
    #fine mia aggiunta
    parser.add_argument(
        '--use-frames',
        default=False,
...
def main():
    ...
    #__saving the values in a external file
    if args.file_url:

        with open(args.file_url, "w") as f:
            for i in range(16):
                f.write(f"Frame {i+1} - Heatmap values:\n")
                f.write(" ".join(map(str, heatmaps_np[0, i, :, :].flatten())) + "\n") 

        print(f'Heatmap values saved to {args.file_url}')
    else: print('Please provide an url for the output file')
